...
def main(opts, cfgs):
    ...
    algo_name = str(opts.tracker)

    """
    1. load model
    """
    app = QtWidgets.QApplication([])
    msg_box1 = MyMessageBox()
    msg_box1.setWindowIcon(QIcon("./pic/logo.png"))
    msg_box1.setWindowTitle("Loading model and Tracking")
    msg_box1.setText("The system is loading the model and \n"
                     f"using {algo_name} algorithm for tracking...\n"
                     "Please wait for a while...")
    msg_box1.setStandardButtons(QMessageBox.NoButton)
    msg_box1.setWindowFlags(msg_box1.windowFlags() | QtCore.Qt.WindowStaysOnTopHint)
    msg_box1.show()
    ...
    if opts.data_format == 'yolo':
        ...
        DATA_ROOT = os.path.join('./test_video', 'sequences')  # Todo：需要删除，否则会越堆越多
    ...
    folder_name = now.strftime("%m_%d_%H_%M")
    folder_name = opts.tracker + '_' + folder_name

    for seq in seqs:
        print(f'--------------tracking seq {seq}--------------')
        path = os.path.join(DATA_ROOT, seq) if opts.data_format == 'origin' else os.path.join('./', f'{opts.dataset}',
                                                                                              'test.txt')
        ...
        for i, (img, img0) in enumerate(data_loader):
            ...
            if opts.save_images:
                plot_img(img0, frame_id, [cur_tlwh, cur_id, cur_cls],
                         save_dir=os.path.join('./test_video', folder_name, seq))
        ...
        if opts.save_images and opts.save_videos:
            save_videos(seq_names=seq, folder_name=folder_name)
    msg_box1.done(0)
    msg_box2 = QMessageBox()
    msg_box2.setWindowIcon(QIcon("./pic/logo.png"))
    msg_box2.setWindowTitle("Success")
    msg_box2.setText("Track completed!")
    msg_box2.setWindowFlags(msg_box1.windowFlags() | QtCore.Qt.WindowStaysOnTopHint)
    msg_box2.show()
    app.exec_()
    ...
    if opts.track_eval:
        ...
        dataset_list = [trackeval.datasets.MotChallenge2DBox(dataset_config)] if opts.dataset in ['mot', 'uavdt'] else [
            trackeval.datasets.VisDrone2DBox(dataset_config)]
        metrics_list = []
...
def save_videos(seq_names, folder_name=None):
    """
    ...
    for seq in seq_names:
        images_path = os.path.join('./test_video', folder_name, seq)
        images_name = sorted(os.listdir(images_path))

        to_video_path = os.path.join(images_path, '../', seq + '.mp4')
        # to_video_path = os.path.join('./tracked_video', folder_name, seq + '.mp4')
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")

        img0 = Image.open(os.path.join(images_path, images_name[0]))
        vw = cv2.VideoWriter(to_video_path, fourcc, 30.0, img0.size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='mydata', help='visdrone, or mot')
